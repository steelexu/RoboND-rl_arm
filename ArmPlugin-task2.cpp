...
#define REWARD_MULTIPLIER 660.0f
		...
		*/
		...
		//if((strcmp(contacts->contact(i).collision1().c_str(), COLLISION_ITEM) == 0))
		{
          ...
          if(episodeFrames < 30){
            ...
            printf("------------Contact to arm other place\n");
          }else {
	...
	if( hadNewState && !newReward )
	{
		...
		*/ 
			...
			if( episodeFrames > 1 )
			{
				...
				const float movingAvg = 0.12f;//0.9f;
				// compute the smoothed moving average of the delta of the distance to the goal
				avgGoalDelta  = (avgGoalDelta * movingAvg) + (distDelta * (1.0f - movingAvg));;
				rewardHistory = avgGoalDelta * REWARD_MULTIPLIER;//exp(-GAMMA_FALLOFF * avgGoalDelta) * 0.1f;//
				newReward     = true;
			}
